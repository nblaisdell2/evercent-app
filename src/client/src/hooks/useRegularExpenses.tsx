...
function useRegularExpenses(widgetProps: WidgetProps) {
  const {
    ...
    budget,
    categoryGroups,
    updateMonthsAhead: updateMonthsAheadFn,
  } = useEvercent();
  if (!categoryGroups) return;

  const updateMonthsAhead = updateMonthsAheadFn();

  const { mutateAsync, error, data } = useSQLMutation2(
    "budget",
  ...
  const postCategoryAmountsToBudget = async () => {
    ...
    // to user's budget
    await utils.user.getAllUserData.invalidate();
