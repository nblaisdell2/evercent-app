...
function useBudgetAutomation(widgetProps: WidgetProps) {
  const {
    ...
    autoRuns,
    pastRuns,
    saveAutoRuns: saveAutoRunsFn,
    cancelAutoRuns: cancelAutoRunsFn,
  } = useEvercent();
  if (!autoRuns || !pastRuns || !categoryGroups) return;

  const saveAutoRuns = saveAutoRunsFn();
  const cancelAutoRuns = cancelAutoRunsFn();
  ...
  const saveAutoRunDetails = async () => {
    ...
    await saveAutoRuns.mutateAsync({
      userID: userData?.userID as string,
      budgetID: userData?.budgetID as string,
      autoRun: autoRunsList[0],
    });
  ...
  const cancelAutomation = async () => {
    widgetProps.setModalIsSaving(true);
    await cancelAutoRuns.mutateAsync({
      userID: userData?.userID as string,
      budgetID: userData?.budgetID as string,
    });
    widgetProps.setModalIsSaving(false);
